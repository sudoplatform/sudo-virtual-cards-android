...
class SudoVirtualCardsUpdateVirtualCardTest : BaseTests() {
    ...
    @Test
    fun `updateVirtualCard() should return success result when no error present`() =
        runBlocking<Unit> {
            val deferredResult =
    ...
    @Test
    fun `updateVirtualCard() should return partial result when unsealing fails`() =
        runBlocking<Unit> {
            mockKeyManager.stub {
    ...
    @Test
    fun `updateVirtualCard() should throw when response is null`() =
        runBlocking<Unit> {
            val mockOperation: GraphQLOperation<String> = mock()
            whenever(
    ...
    @Test
    fun `updateVirtualCard() should throw when mutation response has an identity verification error`() =
        runBlocking<Unit> {
            val errors =
                listOf(
    ...
    @Test
    fun `updateVirtualCard() should throw when response has a card not found error`() =
        runBlocking<Unit> {
            val errors =
                listOf(
                    GraphQLResponse.Error(
    ...
    @Test
    fun `updateVirtualCard() should throw when response has an invalid card state error`() =
        runBlocking<Unit> {
            val errors =
                listOf(
                    GraphQLResponse.Error(
    ...
    @Test
    fun `updateVirtualCard() should throw when current key pair retrieval returns null`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `updateVirtualCard() should throw when unsealing fails`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `updateVirtualCard() should throw when http error occurs`() =
        runBlocking<Unit> {
            val errors =
                listOf(
                    GraphQLResponse.Error(
    ...
    @Test
    fun `updateVirtualCard() should throw when unknown error occurs()`() =
        runBlocking<Unit> {
            mockApiCategory.stub {
                on {
                    mutate<String>(
    ...
    @Test
    fun `updateVirtualCard() should not block coroutine cancellation exception`() =
        runBlocking<Unit> {
            mockApiCategory.stub {
                on {
                    mutate<String>(
        ...
        verify(mockApiCategory).mutate<String>(
            check {
                ...
                mutationInput
                    ...
                    ?.getOrNull()
                    ?.addressLine2
                    ?.getOrNull() shouldBe "addressLine2"
