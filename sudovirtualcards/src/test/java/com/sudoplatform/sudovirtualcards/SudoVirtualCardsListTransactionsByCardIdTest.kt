...
class SudoVirtualCardsListTransactionsByCardIdTest : BaseTests() {
    ...
    private val mockApiCategory by before {
        mock<ApiCategory>().stub {
            on {
                query<String>(
                    argThat { this.query.equals(ListTransactionsByCardIdQuery.OPERATION_DOCUMENT) },
                    any(),
                    any(),
                )
            } doAnswer {
    ...
    @Test
    fun `listTransactionsByCardId() should return success result when no error present`() =
        runBlocking<Unit> {
            val deferredResult =
            ...
            verifyListTransactionsByCardIdQuery("cardId", 1, null, true)
    ...
    @Test
    fun `listTransactionsByCardId() should return success result using default inputs when no error present`() =
        runBlocking<Unit> {
            val deferredResult =
                async(Dispatchers.IO) {
            ...
            verifyListTransactionsByCardIdQuery("cardId", 100)
            verify(mockKeyManager, times(18)).decryptWithPrivateKey(anyString(), any(), any())
            verify(mockKeyManager, times(18)).decryptWithSymmetricKey(any<ByteArray>(), any<ByteArray>())
        }

    @Test
    fun `listTransactionsByCardId() should return success result when populating nextToken`() =
        runBlocking<Unit> {
            val mockOperation: GraphQLOperation<String> = mock()
            whenever(
            ...
            val deferredResult =
                async(Dispatchers.IO) {
                    client.listTransactionsByCardId("cardId", 1, "dummyNextToken")
                }
            deferredResult.start()
            ...
            verifyListTransactionsByCardIdQuery("cardId", 1, "dummyNextToken")
            verify(mockKeyManager, times(18)).decryptWithPrivateKey(anyString(), any(), any())
            verify(mockKeyManager, times(18)).decryptWithSymmetricKey(any<ByteArray>(), any<ByteArray>())
        }

    @Test
    fun `listTransactionsByCardId() should return success empty list result when query result data is empty`() =
        runBlocking<Unit> {
            val mockOperation: GraphQLOperation<String> = mock()
            whenever(
                mockApiCategory.query<String>(
    ...
    @Test
    fun `listTransactionsByCardId() should return success empty list result when query response is null`() =
        runBlocking<Unit> {
            val mockOperation: GraphQLOperation<String> = mock()
            whenever(
                mockApiCategory.query<String>(
    ...
    @Test
    fun `listTransactionsByCardId() should return partial results when unsealing fails`() =
        runBlocking<Unit> {
            mockKeyManager.stub {
    ...
    @Test
    fun `listTransactionsByCardId() should not return duplicate transactions with matching identifiers`() =
        runBlocking<Unit> {
            val mockOperation: GraphQLOperation<String> = mock()
            whenever(
                mockApiCategory.query<String>(
    ...
    @Test
    fun `listTransactionsByCardId() should throw when unsealing fails`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `listTransactionsByCardId() should throw when http error occurs`() =
        runBlocking<Unit> {
            val errors =
                listOf(
    ...
    @Test
    fun `listTransactionsByCardId() should throw when unknown error occurs()`() =
        runBlocking<Unit> {
            mockApiCategory.stub {
                on {
                    query<String>(
    ...
    @Test
    fun `listTransactionsByCardId() should not block coroutine cancellation exception`() =
        runBlocking<Unit> {
            mockApiCategory.stub {
                on {
                    query<String>(
    ...
    private fun verifyListTransactionsByCardIdQuery(
        ...
        verify(mockApiCategory).query<String>(
            check {
                assertEquals(ListTransactionsByCardIdQuery.OPERATION_DOCUMENT, it.query)
